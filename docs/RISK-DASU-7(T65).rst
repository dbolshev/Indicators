...
Розрахунок індикатора вимикається після того, як процедура завершується ``data.status = 'complete'``.
...
Якщо процедура знаходиться у статусі ``data.status = 'active.qualification'`` або ``data.status = 'active.awarded'``та в аналітичній таблиці tbl_winnedLotsWithDisquals відповідно по ``data.id`` процедури знаходиться рядок, де один Постачальник виграв від 5 лотів, то значення індикатора стає "``1``".
