=====================================================================================================================
RISK_DASU-11(Т67). Закупівля близька до порогу визначенного Законом ('belowThreshold', товари/послуги, 5% нижче 200К)
=====================================================================================================================
...
---------------------------------------
 Для процедур типу *допорогові закупівлі* розрахунок індикатора вимикається тоді, коли закінчується період подачі пропозицій. Для процедур типу *процедура звітування* розрахунок індикатора вимикається тоді, коли за моменту завершення процедури (``data.date``) проходить 2 доби.
...
2.а) Якщо очікувана вартість в гривнях перевищує 190000 (сто дев'яносто тисяч) і менше 200000 (двісті тисяч) та в аналітичній таблиці є запис для поточного замовника (конкатенація ``data.procuringEntity.identifier.scheme`` та ``data.procuringEntity.identifier.id``) то індикатор приймає значення "1".
...
3.а) Якщо очікувана вартість в гривнях перевищує 950000 (дев'ятьсот п'ятдесят тисяч) і менще 1000000 (один мільйон) та в аналітичній таблиці є запис для поточного замовника (конкатенація ``data.procuringEntity.identifier.scheme`` та ``data.procuringEntity.identifier.id``), то індикатор приймає значення "1".
