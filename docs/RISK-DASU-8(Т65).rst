=========================================================================================================
RISK-DASU-8(Т65). Переможець торгів виграв усі лоти тендеру, де були дискваліфікації (тендер на 5+ лотів)
=========================================================================================================
...
1. У процедурі має бути від 5-ти лотів, тобто кількість об'єктів з різними ``data.lots.id`` >= 5.

2. Усі активні лоти має виграти лише один учасник, тобто всі пари ``data.awards.suppliers.identifier.scheme`` та ``data.awards.suppliers.identifier.id`` з об'єктів, де ``data.awards.status = 'active'``, мають співпадати.

3. У процедурі мають бути дискваліфікації, тобто має бути хоч один об'єкт визначення переможця, де ``data.awards.status = 'unsuccessful'``.

4. Якщо виконуються попередні три умови, індикатор приймає значення ``1``.
