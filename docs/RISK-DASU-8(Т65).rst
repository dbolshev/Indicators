...
Якщо процедура знаходиться у статусі ``data.status = 'active.awarded'`` та в аналітичній таблиці tbl_winnedLotsWithDisquals відповідно по ``data.id`` процедури знаходиться лише один рядок, тобто серед перможців є всього один постачальник, кількість лотів процедури більше або дорівнює 5, а значення "Дискваліфікації" у рядку дорівнює "1", значення індикатора стає "``1``".
