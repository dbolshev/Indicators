============================================================================
RISK-DASU-25(T120). Зміни до контракту внесені одразу зразу після підписання
============================================================================
...
2. Для кожного об'єкту з модуля контратингу визначаємо його ``data.dateSigned``.
3. Серед документів ``data.documents`` знаходимо всі документи, що мають ``data.documents.documentOf = 'change'`` та ``data.documents.format != 'application/pkcs7-signature'``.
4. Для документів з пункту 3 знаходимо найранішу дату публікації ``data.documents.datePublished``. Якщо вона відрізняється від ``data.dateSigned`` менше ніж на 30 днів, то індикатор приймає значення ``1``. 
