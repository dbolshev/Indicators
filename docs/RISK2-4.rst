...
2. У процедурі кількість об'єктів з різними ``data.lots.id`` < 5 або у процедурі немає дискваліфікацій, тобто немає жодного об'єкту ``data.awards.status = 'unsuccessful'``, індикатор приймає значення ``-2``.

3. Якщо у процедурі є від 5-ти лотів, тобто кількість об'єктів з різними ``data.lots.id`` >= 5 та у процедурі є дискваліфікації, тобто має бути хоч один об'єкт визначення переможця, де ``data.awards.status = 'unsuccessful'``, переходимо до наступного кроку.

4. Якщо усі активні лоти виграв лише один учасник, тобто всі пари ``data.awards.suppliers.identifier.scheme`` та ``data.awards.suppliers.identifier.id`` з об'єктів, де ``data.awards.status = 'active'``, мають співпадати - індикатор приймає значення ``1``. В іншому випадку індикатор приймає значення ``0``.
