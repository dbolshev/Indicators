############################################################
DASU-3-1 Документи тендера не позначені електронним підписом
############################################################
...
Ненакладення ЕЦП на документи тендера є порушенням абзацу другого п.2 `наказу МЕРТ №490 від 22.03.2016 <http://zakon2.rada.gov.ua/laws/show/z0449-16>`_.
...
Для розрахунку індикатора використвоуються наступні поля с API модуля тендеринга:
...
1. Усі документи з блоку ``data.documents`` перевіряються на предмет їх формату ``data.documents.format``

2. Якщо в блоці ``data.documents`` нема жодного документу з форматом ``application/pkcs7-signature``, то індикатор приймає значення ``1``. Якщо такий документ є, то індикатор приймає значення ``0``.
