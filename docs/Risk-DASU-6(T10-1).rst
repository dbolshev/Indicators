...
- аналітична таблиця   для всіх учасників, що беруть участь у закупівлях на дату розрахунку. В таблицю заносяться усі ``data.ProcuringEntity.identifier.id`` Замовників процедур (зі статусами ``belowThreshold``, ``aboveThresholdUA`` та ``aboveThresholdEU``), у яких брав участь Учасник (``data.bids.status = 'active'``).
...
Подія, що вмикає розрахунок індикатора - завершення періоду подачі заявок на участь в аукціоні, що відображується переході значення змінної ``data.status`` у значення ``active.auction``.
