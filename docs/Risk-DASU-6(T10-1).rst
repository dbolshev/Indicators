...
Для кожного Учасника, що має свій ``data.bids.tenderers.identifier.id``, та для якого ``data.bids.status = 'active'`` перевіряємо список Замовників з таблиці tbl_suppliersSingleBidder. Якщо він єдиний та його id дорівнює ``data.ProcuringEntity.identifier.id`` процедури, то індикатор приймає значення "``1``".
